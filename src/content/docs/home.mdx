...
- [Job Workflow](/images/diagrams/eval-hub-eval-job-flow.html) - shows the flow of data through the system when an evaluation job is run.
- [MCP EDD Workflow](/images/diagrams/eval-hub-mcp-edd-flow.html) - shows the flow of data through the system when an MCP EDD job is run.

## Available Adapters

Community providers with a `provider.yaml` are listed in the [Provider Catalog](/providers/catalog/) — filter, inspect benchmarks, and download Kubernetes ConfigMaps.
...
<CardGrid>
  ...
  <LinkCard title="MCP support" href="/mcp/index" description="Exposing EvalHub as a MCP server" />
  <LinkCard title="Agent Discoverability" href="/mcp/agent-discoverability/" description="How AI agents choose and interpret evaluations" />
</CardGrid>
