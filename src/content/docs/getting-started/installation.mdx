...
The `--local` flag disables authentication and enables CORS, which is required for
...
```bash
PORT=8090 ./bin/eval-hub --local
```

**Persistent storage:** By default the server uses in-memory SQLite and all data is
lost when the process stops. To persist data across restarts, point `DB_URL` at a file:

```bash
DB_URL="file:./data/eval-hub.db?cache=shared" ./bin/eval-hub --local
```
