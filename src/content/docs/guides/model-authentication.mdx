...
mysvc_url: https://mysvc...`"]
    ...
    IS["`**Internal Secret** (ephemeral, per-job)
api-key: api-key:ref
mysvc_api-key: mysvc_api-key:ref
mysvc_url: http://localhost:8080`"]
...
ca_cert: PEM`"]
    ...
    US -->|"`EvalHub generates
ref secret`"| IS
    US -->|"mounted, real values"| SC
    IS -->|"projected volume"| AD
    US -.->|"`passthrough:
hf-token, ca_cert`"| AD
```
