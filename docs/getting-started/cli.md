...
EvalHub is assumed to be running on your OpenShift cluster. If it is not, see [Installation](installation.md) first.
...
If the model endpoint requires authentication, see [Model authentication](model-authentication.md).
...
- [Python SDK](../reference/sdk-client.md) — programmatic access with the same capabilities
- [Model authentication](model-authentication.md) — API keys, CA certs, and ServiceAccount tokens
- [Installation](installation.md) — server deployment on OpenShift
